...
public class TargetStatusPoller {
	...
	public void startPolling() {
		...
		timer.schedule(pollerTask , (long)(intervalInMinutes * 60 * 1000));//pollerTask를 일정시간동안 스케쥴 함
	}
	...
	private void pollAgain() {
		if (continuePolling){//만약 polling 중이면
			System.out.println(".. polling again .. ");
			//polling을 안할때
			pollerTask = new PollerTask();//새로 poll을 한다. 
    ...
    class PollerTask extends TimerTask {//
        public void run() {
        	try {
            	TargetState targetState = TargetState.createFromJSON( getTarget() );//Json에서 target정보가 담긴 targetState을 얻어낸다.
				
				targetStatusListener.OnTargetStatusUpdate( targetState );
				
			} catch ( URISyntaxException | IOException e) {//에러가 있으면 타이머를 중단시키고 종료
				timer.cancel();
				e.printStackTrace();
				return;
			}
