...
public class MapNav : MonoBehaviour
{
    ...
    public float fixLat = 37.38174f;                            //Latitude
    public float fixLon = 127.1304f;                            //Longitude
    ...
    public void refreshMapPosition(){
        StartCoroutine(MapPosition());
    }
    ...
    IEnumerator MapPosition()
    {
        ...
        Debug.Log("dad? " + user.position.x + " " + iniRef.x + " " + user.position.z + " " + iniRef.z);
        ...
        //        url = "http://maps.googleapis.com/maps/api/staticmap?center=" + fixLat + "," + fixLon + "&zoom=" + zoom + " &scale=2&size=1280x1280&format=" + mapFormat[indexFormat] + "&maptype=" + maptype[indexType] + "&sensor=false&key=" + key;
        url = "http://maps.googleapis.com/maps/api/staticmap?center=" + fixLat + "," + fixLon + "&zoom=" + zoom + "&scale=2&size=1280x1280&format=" + "png" + "&maptype=" + "roadmap" + "&sensor=false";//&key=" + key;
    ...
    IEnumerator Online()
    {
        if (!mapDisabled)
        {
            ...
            //원래는 안전성? 을 위해서 아래것으로 해야하지만 계속 에러가 나서 다 생략하고 위의 방식으로 함!
            /*
